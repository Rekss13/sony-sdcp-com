...
const { SdcpClient, aspectRatio } = require('../index.js');
...
describe('Connect to projector', function () {
    ...
    after((done) => {
        (server && server.server) ? server.server.close(done) : done();
    });
    ...
    it('should turn on power and return correct status', (done) => {
        ...
        client.setPower(true)
            .then(status => assert.equal(status, 'ON'))
            .then(done, done)
    });
    ...
    it('should return power status OFF', (done) => {
        ...
        client.getPower()
            .then(status => assert.equal(status, 'OFF'))
            .then(done, done);
    });
    ...
    it('should return aspect ratio', (done) => {
        ...
        client.getAspectRatio()
            .then(status => assert.equal(status, 'ZOOM_2_35'))
            .then(done, done);
    });
    ...
    it('set aspect ratio and return correct status', (done) => {
        ...
        client.setAspectRatio(aspectRatio.STRETCH)
            .then(status => assert.equal(status, 'STRETCH'))
            .then(done, done);
    });
});
...
describe('When unable to connect', function () {
    ...
    it('should emit connection refused error', (done) => {
